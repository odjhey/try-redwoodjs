...
import { QUERY } from 'src/components/Admin/Unit/UnitsCell'
    ...
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        ...
        <tbody>
          {units.map((unit) => (
            <tr key={unit.id}>
              <td>{truncate(unit.id)}</td>
              <td>{truncate(unit.name)}</td>
              <td>{truncate(unit.desc)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.adminUnit({ id: unit.id })}
                    title={'Show unit ' + unit.id + ' detail'}
