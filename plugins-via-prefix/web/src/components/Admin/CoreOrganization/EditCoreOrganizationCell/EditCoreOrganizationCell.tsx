...
import CoreOrganizationForm from 'src/components/Admin/CoreOrganization/CoreOrganizationForm'
...
const UPDATE_CORE_ORGANIZATION_MUTATION = gql`
  mutation UpdateCoreOrganizationMutation(
    $id: Int!
...
export const Success = ({
  coreOrganization,
}: CellSuccessProps<EditCoreOrganizationById>) => {
  const [updateCoreOrganization, { loading, error }] = useMutation(
    UPDATE_CORE_ORGANIZATION_MUTATION,
    {
      onCompleted: () => {
        toast.success('CoreOrganization updated')
        navigate(routes.adminCoreOrganizations())
      },
      onError: (error) => {
    ...
    <div className="rw-segment">
      <header className="rw-segment-header">
        <h2 className="rw-heading rw-heading-secondary">
          Edit CoreOrganization {coreOrganization.id}
        </h2>
      </header>
      <div className="rw-segment-main">
        <CoreOrganizationForm
